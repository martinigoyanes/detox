...
class GediAdapter(GenerationMixin):
    def __init__(
        ...
        self.paraphraser = AutoModelForSeq2SeqLM.from_pretrained(paraphraser_name)
        self.paraphraser_tokenizer = AutoTokenizer.from_pretrained(paraphraser_name)
        self.gedi_model = AutoModelForCausalLM.from_pretrained(gedi_name)
        self.clf = RobertaForSequenceClassification.from_pretrained(clf_name) if clf_name else None
    ...
    def _setup(self, gedi_name):
        ...
        if os.path.exists(gedi_name):
            ...
            self.gedi_model.bias = w['bias']
            self.gedi_model.logit_scale = w['logit_scale']
            del w
        else:
            self.gedi_model.bias = torch.tensor([[ 0.08441592, -0.08441573]])
            self.gedi_model.logit_scale = torch.tensor([[1.2701858]])
    ...
    def paraphrase(self, texts, max_length=128, beams=10):
        ...
        inputs = self.paraphraser_tokenizer(texts, return_tensors='pt', padding=True)['input_ids']
        if max_length == 'auto':
